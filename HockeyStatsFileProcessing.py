    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS "{}" (     
                ...
                PRIMARY KEY(player_name, number),       
                FOREIGN KEY (team_name) REFERENCES KIHF(team_name)             
            )
        ...
        try:
           ...
           gka1_number = int(list_df[8].iat[0, 0]) 
         
        try:
            gka1_mip,y = list_df[12].iat[0, 1].split(":")
        except Exception as e:
        
            gka1_mip,y = list_df[12].iat[0, 2].split(":")
            
        try:
            gka1_ga = int(list_df[12].iat[0, 2])
        except:
            gka1_ga = int(list_df[12].iat[0, 3])
        ...
        print("IT BROKE", e, game_id, " ", game_no)
        # Capture and print/log the exception with line number
        logging.error(f"IT BROKE {e} {game_id} {game_no}")
        logging.error("Exception occurred", exc_info=True)  # This logs the traceback
        print("IT BROKE", e, game_id, game_no)       
